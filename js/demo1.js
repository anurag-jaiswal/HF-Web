 ...
 */
{
	...
	const TiltObj = function(el, options) {
		...
		this.DOM = {};
		this.DOM.img = this.el.querySelector('.work__img');
		this.DOM.title = this.el.querySelector('.work__title');
		this._initEvents();
	};
	...
	DOM.workElems = Array.from(document.querySelectorAll('.work-wrap'));
	DOM.workLinks = Array.from(document.querySelectorAll('.work__link'));
	DOM.footer = document.querySelector('.work--related');
	const workElemsTotal = DOM.workElems.length;
	const shapes = [
		{
	...
	const createScrollWatchers = function() {
		DOM.workElems.forEach((el,pos) => {
			const scrollElemToWatch = pos ? DOM.workElems[pos] : DOM.footer;
			pos = pos ? pos : workElemsTotal;
			...
			watcher.exitViewport(function() {
				...
				if( idx <= workElemsTotal && step !== idx ) {
					step = idx;
					anime.remove(DOM.shapeEl);
					anime({
						targets: DOM.shapeEl,
